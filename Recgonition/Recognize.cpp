...
{
    QString folder_path = ":/Data/Test";
    QString file_path = folder_path + "/" + QString::fromStdString(test_image_name);
    QImage img(file_path);
    img = img.convertToFormat(QImage::Format_BGR888);
    Mat test_image = Mat(img.height(), img.width(), CV_8UC3, img.bits(), img.bytesPerLine());
